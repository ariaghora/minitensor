...
MTTensor *mt_alloc_empty_tensor(MTContext *ctx) {
    MTTensor *t = newptr(MTTensor, 1);
    t->context  = ctx;
    t->data     = NULL;
    t->datalen  = 0;
    t->deps     = newptr(MTTensor *, INITIAL_N_DEPS);
    t->grad     = NULL;
    t->grad_fn  = NULL;
    t->indices  = NULL;
    t->isleaf   = 1;
...
inline void __init_indices(MTTensor *t) {
    ...
    t->indices = newptr(int *, t->ndims);
    for (int i = 0; i < t->ndims; i++) {
        int *idx = newptr(int, t->shape[i]);
...
MTTensor *mt_new_tensor(MTContext *context,
    ...
    t->data     = newptr(float, datalen);
    t->datalen  = datalen;
    t->ndims    = ndims;
    t->shape    = newptr(int, ndims);
...
MTTensor *mt_new_tensor_full(MTContext *ctx, float val,
                             int *shape, int ndims) {
    int    datalen = __prod(shape, ndims, int);
    float *data    = newptr(float, datalen);
...
IdxIterator *mt_new_idxiterator(int **indices, int *shape, int ndims) {
    IdxIterator *it = newptr(IdxIterator, 1);
    it->ndims       = ndims;
    it->nextidx     = newptr(int, ndims);
    it->nextactidx  = newptr(int, ndims);
    it->indices     = indices;
    it->shape       = shape;
    it->i           = -1;
    it->nxt         = 0;
...
int *mt_idxiterator_next(IdxIterator *it) {
    ...
    if (it->nxt < 0) EXIT_WITH_ERROR("trying to access out-of-bound indices");
...
MTTensor *mt_tensor_slice(MTContext *ctx, MTTensor *t, int dim,
    ...
    int **newindices = newptr(int *, t->ndims);
    int  *newshape   = newptr(int, t->ndims);
    for (int i = 0; i < t->ndims; i++) {
        if (i == dim) {
            newshape[i]   = indexlen;
            newindices[i] = index;
...
void mt_tensor_free(MTTensor *t) {
    if (t != NULL) {
        // null the index of node's parent's children that points to this node
        int idxpchild = -1;
        if (t->parent != NULL)
            idxpchild = __find_in_list(t->parent->deps, t, t->parent->ndeps);
...
void mt_context_defrag(MTContext *ctx) {
    MTTensor **newtrackedptr = newptr(MTTensor *, ctx->ntracked);

    int cnt = 0;
    for (int i = 0; i < ctx->ntracked; i++)
        if (ctx->tracked[i] != NULL) newtrackedptr[cnt++] = ctx->tracked[i];
    // free the old `tracked` pointers
    free(ctx->tracked);
    ctx->ntracked = cnt;
    ctx->tracked  = newtrackedptr;
...
MTContext *mt_new_context(void) {
    MTContext *ctx = newptr(MTContext, 1);
    ctx->withgrads = 0;
    ctx->ntracked  = 0;
    ctx->cap       = INITIAL_CAP;
    ctx->tracked   = newptr(MTTensor *, INITIAL_CAP);
    return ctx;
}
