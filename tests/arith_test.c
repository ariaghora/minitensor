...
void run_tensor_addition_tests(Test *t) {
    ...
    MTTensor  *z         = new_tensor(ctx, Arr(float, 0, 0, 0, 0), Arr(int, 2, 2), 2);
    MTTensor  *expected1 = new_tensor(ctx, Arr(float, 2, 4, 6, 8), Arr(int, 2, 2), 2);
    MTTensor  *expected2 = new_tensor(ctx, Arr(float, 1, 2, 3, 4), Arr(int, 2, 2), 2);

    MTTensor *result1 = tensor_add(x, y);
    MTTensor *result2 = tensor_add(x, z);
    ...
    MTTensor *sc1       = new_scalar(ctx, 2);
    MTTensor *sc2       = new_scalar(ctx, 3);
    MTTensor *result3   = tensor_add(x, sc1);
    MTTensor *expected3 = new_tensor(ctx, Arr(float, 3, 4, 5, 6), Arr(int, 2, 2), 2);

    assert_true(t, mt_is_tensor_eq(result3, expected3), "test tensor-scalar addition", "tensor-scalar addition result is wrog");
    assert_true(t, mt_is_tensor_eq(tensor_add(sc1, sc2), new_scalar(ctx, 5)), "test scalar-scalar addition", "scalar-scalar addition result is wrog");
...
void run_tensor_subtraction_tests(Test *t) {
    ...
    MTTensor *result1 = tensor_sub(x, y);
    MTTensor *result2 = tensor_sub(x, z);
...
void run_tensor_reduce_tests(Test *t) {
    ...
    MTTensor  *exp1    = new_tensor(ctx, Arr(float, 9, 12), Arr(int, 1, 2), 2);
    MTTensor  *exp2    = new_tensor(ctx, Arr(float, 3, 7, 11), Arr(int, 3), 1);
