...
void run_simple_autograd_tests(Test *t) {
    ...
    MTTensor *y = new_tensor(ctx, Arr(float, 2, 4, 6, 8), Arr(int, 2, 2), 2);
    tensor_enable_grad(y);

    // test if at least one of operand requires grad, then the the result
    // also requires grad
    MTTensor *res = tensor_add(x, y);
...
void run_autograd_backward_tests(Test *t) {
    MTContext *ctx = new_context();
    MTTensor  *x   = new_tensor(ctx, Arr(float, 1, 2, 3), Arr(int, 3, 1), 2);
    MTTensor  *y   = new_tensor(ctx, Arr(float, 1, 2, 3), Arr(int, 3, 1), 2);
    tensor_enable_grad(x);
    tensor_enable_grad(y);

    MTTensor *expgrad1 = new_tensor(ctx, Arr(float, 1, 1, 1), Arr(int, 3, 1), 2);

    MTTensor *sum = tensor_sum(x, -1);
    ...
    MTTensor *expgrad2 = new_tensor(ctx, Arr(float, 3, 3, 3), Arr(int, 3, 1), 2);
    sum                = tensor_sum(y, -1);
    tensor_backward(sum, mt_new_scalar(ctx, 3.0));
