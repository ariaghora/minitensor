...
void run_tensor_creation_tests(Test *t) {
    MTContext *ctx = new_context();
    MTTensor  *x = new_tensor(ctx, Arr(float, 1, 2, 3, 4), Arr(int, 2, 2), 2);
    MTTensor  *y = new_tensor(ctx, Arr(float, 1, 2, 3, 4), Arr(int, 2, 2), 2);
    MTTensor  *z = new_tensor(ctx, Arr(float, 3, 3, 3, 3), Arr(int, 2, 2), 2);
    ...
    assert_true(t, arrsame(x->indices[0], Arr(int, 0, 1), 2), "test initial 1st axis indices", "indices must be [0, 1]");
    assert_true(t, arrsame(x->indices[1], Arr(int, 0, 1), 2), "test initial 2nd axis indices", "indices must be [0, 1]");

    // scalar
    MTTensor *sc = new_scalar(ctx, 42);
    assert_true(t, sc->ndims == 0, "test scalar dims", "scalar dimension must be 0");

    // full tensor
    MTTensor *full = new_tensor_full(ctx, 3, Arr(int, 2, 2), 2);
    MTTensor *expected_full = new_tensor(ctx, Arr(float, 3, 3, 3, 3), Arr(int, 2, 2), 2);
    ...
    x = new_tensor(ctx, Arr(float, 1, 2, 3, 4, 5, 6, 7, 8, 9), Arr(int, 3, 3), 2);
...
void run_tensor_access_tests(Test *t) {
    MTContext *ctx = new_context();
    MTTensor  *scal = new_scalar(ctx, 42);
    MTTensor  *x = new_tensor(ctx, Arr(float, 1, 2), Arr(int, 2), 1);
    MTTensor  *y = new_tensor(ctx, Arr(float, 1, 2, 3, 4), Arr(int, 2, 2), 2);
    MTTensor  *z = new_tensor(ctx, Arr(float, 1, 2, 3, 4, 5, 6, 7, 8), Arr(int, 2, 2, 2), 3);
...
void run_tensor_slice_tests(Test *t) {
    MTContext *ctx = new_context();
    MTTensor  *x = new_tensor(ctx, Arr(float, 1, 2, 3, 4, 5, 6), Arr(int, 3, 2), 2);
    MTTensor  *y = new_tensor(ctx, Arr(float, 1, 2, 3, 4, 5, 6, 7, 8), Arr(int, 2, 2, 2), 3);
    ...
    assert_true(t, is_tensor_eq(slice1, exp1), "test slice 2-tensor 1", "the value should be [3, 4]");
    assert_true(t, is_tensor_eq(slice2, exp2), "test slice 2-tensor 2", "the value should be [1, 2, 5, 6]");
    assert_true(t, is_tensor_eq(slice3, exp3), "test slice 3-tensor 1", "the value should be [1, 2, 5, 6]");
    assert_true(t, is_tensor_eq(slice4, exp4), "test slice 3-tensor 2", "the value should be [1, 2, 5, 6]");
...
void run_context_tests(Test *t) {
    MTContext *ctx = new_context();
    MTTensor  *a = new_scalar(ctx, 1.0);
    MTTensor  *b = new_scalar(ctx, 10.0);
    MTTensor  *c = new_tensor(ctx, Arr(float, 1, 2), Arr(int, 2, 1), 2);
    MTTensor  *d = new_tensor(ctx, Arr(float, 2, 4), Arr(int, 2, 1), 2);
